...
struct ips {
    ...
    /* Aho-Corasick multi-pattern matcher for fast signature matching */
    ac_matcher_t *ac_matcher;
    u32 *ac_matches;        /* Pre-allocated match array */
    u32 ac_matches_capacity;
};
...
ips_t *ips_create(void)
{
    ...
    /* Initialize Aho-Corasick matcher for multi-pattern matching */
    ips->ac_matcher = ac_matcher_create(MAX_SIGNATURES);
    if (!ips->ac_matcher) {
    ...
    ips->ac_matches_capacity = 64;
    ips->ac_matches = ngfw_malloc(sizeof(u32) * ips->ac_matches_capacity);
    if (!ips->ac_matches) {
        ac_matcher_destroy(ips->ac_matcher);
        hash_destroy(ips->signatures);
        ngfw_free(ips);
        return NULL;
    }
...
ngfw_ret_t ips_start(ips_t *ips)
{
    ...
    ac_matcher_build(ips->ac_matcher);
    
    ips->running = true;

    log_info("IPS started with AC matcher: %u nodes, %u signatures",
             ac_matcher_get_node_count(ips->ac_matcher),
             ac_matcher_get_signature_count(ips->ac_matcher));
...
ngfw_ret_t ips_add_signature(ips_t *ips, ips_signature_t *sig)
{
    ...
    /* Add to Aho-Corasick matcher for multi-pattern matching */
    if (pat_type == PATMATCH_EXACT) {
        ac_matcher_add_pattern(ips->ac_matcher, (const u8 *)sig->pattern, 
                               (u32)strlen(sig->pattern), sig->id);
    }
...
ngfw_ret_t ips_check_packet(ips_t *ips, packet_t *pkt, ips_alert_t *alert)
{
    ...
    /* First, use Aho-Corasick for fast multi-pattern matching */
    u32 match_count = 0;
    ngfw_ret_t ret = ac_matcher_find(ips->ac_matcher, data, data_len,
                                      ips->ac_matches, &match_count, 
                                      ips->ac_matches_capacity);
    
    if (ret == NGFW_OK && match_count > 0) {
        /* Found matches via AC - process first match */
        u32 sig_id = ips->ac_matches[0];
        ips_signature_impl_t *sig = hash_lookup(ips->signatures, &sig_id);
        
        if (sig && sig->sig.enabled) {
            ips->stats.matches++;
    ...
    /* Fall back to individual pattern matching for wildcards/regex */
    ips_signature_impl_t *sig = ips->sig_list;
    while (sig) {
        if (!sig->sig.enabled) {
            sig = sig->next;
        ...
        /* Skip exact patterns - already checked by AC matcher */
        bool is_exact = true;
        const char *p = sig->sig.pattern;
        while (*p) {
            if (*p == '*' || *p == '?') { is_exact = false; break; }
            p++;
        }
        if (is_exact) {
            sig = sig->next;
